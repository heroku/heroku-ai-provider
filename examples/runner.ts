...
async function runExample() {
  ...
  try {
    const exampleModule = await import(resolve(process.cwd(), 'examples', fileName));
    
    if (typeof exampleModule[functionName] !== 'function') {
      console.error(`Function '${functionName}' not found in ${fileName}`);
      console.error('Available functions:', Object.keys(exampleModule).filter(key => typeof exampleModule[key] === 'function'));
      process.exit(1);
    }
